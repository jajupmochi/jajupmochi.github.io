...
- [Hard rules](#hard-rules)
    - Ask before doc updates — batched, not per-edit
    - Ask before SEO audit — long reports bloat context
    - JSON validity, i18n parity, bilingual docs, content source of truth, Master TOC requirement
...
- **Blog writing style (added 2026-05-28).** When drafting or editing ANY `blog-posts/**` content, Claude MUST follow [`docs/conventions/blog-writing-style.md`](docs/conventions/blog-writing-style.md) (中文: `blog-writing-style.zh.md`). Key rules: problem-first first-person opening; natural, de-AI'd, non-verbose prose; multi-level TOC + copy-pasteable detail; document our own config/design (what/why/where) and how readers can reproduce it; an AI prompt + agent-read-this-URL paragraph at the top; distinguish the human author from Claude in every personal reference; Mermaid/SVG diagrams; ship zh + en files with honest `translationStatus`. Distilled from Linlin's instructions in the 杂活/chores_by_claude session.
- **Ask before doc updates (added 2026-04-21).** The two mandatory rules below still apply per edit batch, **but do not auto-apply them mid-session.** At the end of each edit batch, ASK Linlin whether to update `docs/UPDATES.md` + `docs/PLAN.md` now or defer. If she defers, keep the pending doc changes as a running list (task notes / scratchpad) and apply them in one consolidated edit when she says "更新文档" / "update docs" / "sync docs". Rationale: per-batch doc updates in a long session create noise — batched updates read cleaner in history and in commits. The backfill-if-missed clause below still applies: by the time a commit goes out, the relevant UPDATES + PLAN entries MUST be in place.
- **Ask before SEO audit (added 2026-04-21).** SEO-related skills / plugins / scripts (`searchfit-seo:seo-audit`, any other SEO check tool, any manual SEO audit of `index_en.html`) MUST NOT auto-invoke even when the task context matches. Always ASK Linlin first ("要不要跑一次 SEO audit？" / "Run an SEO audit?") and wait for a yes. Reason: SEO audits produce long reports and re-read many files, which bloats context and hastens Claude Code's auto-compaction — Linlin wants to decide explicitly when to pay that cost. This rule overrides the general "tool proactivity" preference in `CLAUDE.local.md` for the SEO case.
- **Use design plugins for design tasks (added 2026-04-27).** When Linlin asks for color / typography / animation / contrast / hierarchy / spacing / hover-state / motion / overall vitality / "more X" or "less X" design changes, the assistant MUST first invoke the matching `impeccable` (or other installed) design subcommand BEFORE writing CSS by hand. Mapping:
  - colors / palette / accent / contrast → `impeccable colorize` or `impeccable bolder`
  - fonts / typography / font hierarchy / handwriting → `impeccable typeset`
  - animations / motion / hover effects / transitions → `impeccable animate`
  - "looks too loud" → `impeccable quieter` · "looks too plain" → `impeccable bolder`
  - "feels old" / "feels generic" → `impeccable critique` first to diagnose, then `bolder` / `colorize` / `animate` based on findings
  - "make it production-ready" → `impeccable polish` or `impeccable harden`
  - layout / spacing / rhythm → `impeccable layout`
  - simplify / strip → `impeccable distill`
  Reason: the plugins encode hundreds of design-system best practices Linlin would otherwise have to re-explain each time. Hand-coded CSS is an escape hatch only when the plugin's recommendation has been seen and rejected. Failure to use them when applicable wastes Linlin's review cycles. This rule extends the "tool proactivity" preference in `CLAUDE.local.md` to design work specifically.
- **Mandatory `docs/UPDATES.md` log.** EVERY change to this repo (content, code, asset, copy, config) MUST add an entry to `docs/UPDATES.md` in the same edit batch. Format: today's date as `# YYYY-MM-DD` H1 (UTC, use `currentDate` from environment or run `date -u +%F`), **newest day on top**. If multiple distinct change-sets land on the same day, group each under `## V1`, `## V2`, `## V3` H2 sub-headings **with the highest V number on top** (most recent work first). Each entry is a short bullet list — what changed and why. Keep the `## Master TOC` at the file's top in sync in the same edit — add a bullet for every new day, and a sub-bullet for every new V with a one-line hook. **A PR / commit without a corresponding `docs/UPDATES.md` entry (and TOC update) is incomplete.** Backfill if missed. Timing: follow the Ask-before-doc-updates rule above — defer until Linlin asks.
- **Mandatory `docs/PLAN.md` sync.** If a change matches an existing `docs/PLAN.md` item, update its status marker (`[ ]`→`[~]`→`[✓]`) in the same edit batch. If it introduces new planned work (not shipping today), add the new Horizon / Milestone / Goal / Task entries with fresh IDs. A PR that changes roadmap-relevant behavior without touching `docs/PLAN.md` is incomplete. Timing: follow the Ask-before-doc-updates rule above — defer until Linlin asks.
